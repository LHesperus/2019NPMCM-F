...
while dij_out.stop~=N
    dij_out=Dijkstra3(graph,graph2,para,cor_pos.type,dij_out.stop,tmp,start_temp,cor_pos.q3);
...
disp(["路中存在的误差点数",sum(cor_pos.q3(path))])
npath=length(path);
type2=zeros(1,npath);
[err3,flag3]= get3_err(cor_pos.x(path),cor_pos.y(path),cor_pos.z(path),cor_pos.type(path),para,type2);
if flag3==0
    disp("路径有效")
else
    disp("路径无效")
